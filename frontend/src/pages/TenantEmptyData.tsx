...
export function TenantEmptyData() {
  ...
  const [isErrorRentalTimeSetting, setIsErrorRentalTimeSetting] =
    useState<boolean>(true);
  const [msgErrorRentalTimeSetting, setMsgErrorRentalTimeSetting] =
    useState<string>(aryErrorMessages[0]);
  ...
  useEffect(() => {
    ...
    function checkRentalTimeSetting() {
      const ownerRentalTime: ITimeZone | null | undefined =
        currentRentalData?.owner_rental_time;
      const bookingTime: ITimeZone[] | undefined =
        currentRentalData?.booking_time;

      // 初期エラーチェック
      if (
        ownerRentalTime === undefined ||
        bookingTime === undefined ||
        ownerRentalTime === null
      ) {
      ...
      const ownerStrTime =
        Number(ownerRentalTime.strTime.split(':')[0]) * 60 +
        Number(ownerRentalTime.strTime.split(':')[1]);
      const ownerEndTime =
        Number(ownerRentalTime.endTime.split(':')[0]) * 60 +
        Number(ownerRentalTime.endTime.split(':')[1]);
      ...
      <ScrollArea w={'100%'} h="calc(100vh - 130px)" px={'6'} py={'2'}>
        ...
        <Box bg={'#F3F7F7'} w={'100%'} borderRadius={'8'} px={'4'} py={'2'}>
          ...
          <HStack gap={'2%'} mb={'4'}>
            <Image
              src={
                import.meta.env.VITE_ORIGIN_API_URL +
                '/images/' +
                currentRentalData?.car_image_url1
              }
              alt={'car'}
              w={'49%'}
            />
            <Image
              src={
                import.meta.env.VITE_ORIGIN_API_URL +
                '/images/' +
                currentRentalData?.car_image_url2
              }
              alt={'car'}
              w={'49%'}
            />
          ...
          >
            <Text w={'30%'} fontSize={'lg'}>
              開始
            </Text>
            <Select
              size="lg"
              w={'30%'}
              items={arySelectableHours}
              value={selectStrHours}
              onChange={(value: string) => handlerSelectedStrHours(value)}
            />
            <Center w={'10%'}>:</Center>
            <Select
              size="lg"
              w={'30%'}
              items={arySelectableMinutes}
              value={selectStrMinutes}
              onChange={(value: string) => handlerSelectedStrMinutes(value)}
            />
          </HStack>
          ...
          >
            ...
            <Select
              ...
              w={'30%'}
              items={arySelectableHours}
              value={selectEndHours}
              onChange={(value: string) => handlerSelectedEndHours(value)}
            />
            <Center w={'10%'}>:</Center>
            <Select
              size="lg"
              w={'30%'}
              items={arySelectableMinutes}
              value={selectEndMinutes}
              onChange={(value: string) => handlerSelectedEndMinutes(value)}
            />
          </HStack>
